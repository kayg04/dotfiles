...
* General Utility
** Undo
- Treat every action as a branching tree of changes instead
  of linear.
#+BEGIN_SRC emacs-lisp
  (use-package undo-tree
    :init
    (setq-default undo-tree-visualizer-diff t)                        ; Show the difference between each node
    (setq-default undo-tree-visualizer-timestamps t)                  ; Show relative timestamps
    (setq-default undo-tree-auto-save-history "~/.emacs.d/undo-tree") ; Save history to a file
    :config
...
python to show results in the interpreter rather than return
statements.
