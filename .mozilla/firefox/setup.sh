...
applyPolicies() {
    ...
    if [[ -d /usr/lib/firefox ]]; then
        FIREFOX_LIBPATH="/usr/lib/firefox/distribution"
        mkdir -p "${FIREFOX_LIBPATH}"
        sudo ln -sf "${SCRIPT_PATH}"/policies.json "${FIREFOX_LIBPATH}"
    elif [[ -d /usr/lib64/firefox ]]; then
        FIREFOX_LIBPATH="/usr/lib64/firefox/distribution"
        mkdir -p "${FIREFOX_LIBPATH}"
        sudo ln -sf "${SCRIPT_PATH}"/policies.json "${FIREFOX_LIBPATH}"
    fi
}
