...
#+BEGIN_SRC conf :tangle etcetera/etc/fstab
  ...
  # Portable Drive
  LABEL=Portable\040Drive    /mnt/backup    btrfs    defaults,nofail,noauto,noatime,nodiratime,ssd,space_cache,compress=zstd,subvol=@backup    0 0
  LABEL=Portable\040Drive    /mnt/vm        btrfs    defaults,nofail,noauto,noatime,nodiratime,ssd,space_cache,nodatacow,subvol=@vm    0 0

  # External Drive I
  LABEL=External\040Drive\040I    /mnt/ext1    btrfs    defaults,nofail,noauto,noatime,nodiratime,compress=zstd    0 0
#+END_SRC
* Firefox
** Profiles
...
#+BEGIN_SRC json :tangle firefox/usr/lib/firefox/distribution/policies.json
  {
    "policies": {
      "CaptivePortal": true,
      "Cookies": {
...
#+BEGIN_SRC sh :tangle scripts/.local/bin/firefox-setup :shebang "#!/usr/bin/env bash"
  # import sanity
  set -euo pipefail
...
- =nounset= ensures all variables have been assigned a value
  ...
  assignment.
#+BEGIN_SRC sh :tangle scripts/.local/bin/helper
  # Bash Sanity
...
#+BEGIN_SRC sh :tangle scripts/.local/bin/wmn :shebang "#!/usr/bin/env bash"
  export SCRIPT_PATH=$(dirname $(realpath "$0"))
