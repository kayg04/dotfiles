...
#+BEGIN_SRC sh :tangle .config/spotify/wmname.sh :shebang "#!/usr/bin/env bash"
  # import sanity
  set -euo pipefail
...
lyrics provider / partner. So I found [[https://github.com/SwagLyrics/SwagLyrics-For-Spotify][this]] which queries the song title from the
running Spotify app, fetches lyrics and opens an issue on Github automatically
if it can't find those lyrics. Pretty cool, right?

#+BEGIN_SRC sh :tangle .config/spotify/lyrics.sh :shebang "#!/usr/bin/env bash"
  # import sanity
  set -euo pipefail
